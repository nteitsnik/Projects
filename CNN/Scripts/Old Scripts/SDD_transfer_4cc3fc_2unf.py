...
class SimpleCNN(nn.Module):
    def __init__(self, num_classes):
        ...
        self.pool4 = nn.MaxPool2d(kernel_size=2, stride=2) ##25x25
        
      

        self.dropout = nn.Dropout(p=0.5)
        self.fc0 = nn.Linear(in_features=32*12*12 , out_features=1024)
    ...
    def forward(self, x):
        ...
        x = self.pool4(F.relu(self.conv4(x)))  # [B, 64, 28, 28]
        
        x = x.view(x.size(0), -1)             # Flatten: [B, 64*28*28]
        x = F.relu(self.fc0(x)) 
        x = self.dropout(x)  
        x = F.relu(self.fc1(x)) 
...
new_num_classes=7
''' 
model.fc1 = nn.Linear(in_features=32 * 12 * 12, out_features=256)
model.to(device)'''
