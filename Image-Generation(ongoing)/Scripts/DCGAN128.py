...
class Generator128Deep(nn.Module):
    def __init__(self, latent_dim=100, img_channels=3, feature_map_size=64):
        super(Generator128Deep, self).__init__()
        self.net = nn.Sequential(
            # Input: (latent_dim, 1, 1)
            nn.ConvTranspose2d(latent_dim, feature_map_size * 16, 4, 1, 0, bias=False),  # -> (1024, 4, 4)
            nn.BatchNorm2d(feature_map_size * 16),
            nn.ReLU(True),

            nn.ConvTranspose2d(feature_map_size * 16, feature_map_size * 8, 4, 2, 1, bias=False),  # -> (512, 8, 8)
            nn.BatchNorm2d(feature_map_size * 8),
            nn.ReLU(True),

            nn.ConvTranspose2d(feature_map_size * 8, feature_map_size * 4, 4, 2, 1, bias=False),  # -> (256, 16, 16)
            nn.BatchNorm2d(feature_map_size * 4),
            nn.ReLU(True),

            nn.ConvTranspose2d(feature_map_size * 4, feature_map_size * 2, 4, 2, 1, bias=False),  # -> (128, 32, 32)
            nn.BatchNorm2d(feature_map_size * 2),
            nn.ReLU(True),

            nn.ConvTranspose2d(feature_map_size * 2, feature_map_size, 4, 2, 1, bias=False),  # -> (64, 64, 64)
            nn.BatchNorm2d(feature_map_size),
            nn.ReLU(True),

            nn.ConvTranspose2d(feature_map_size, feature_map_size // 2, 4, 2, 1, bias=False),  # -> (32, 128, 128)
            nn.BatchNorm2d(feature_map_size // 2),
            nn.ReLU(True),

            nn.ConvTranspose2d(feature_map_size // 2, img_channels, 3, 1, 1, bias=False),  # refine final output
...
class Discriminator128Deep(nn.Module):
    def __init__(self, img_channels=3, feature_map_size=64):
        super(Discriminator128Deep, self).__init__()

        self.net = nn.Sequential(
            # Input: (batch, 3, 128, 128)
            nn.Conv2d(img_channels, feature_map_size, kernel_size=4, stride=2, padding=1, bias=False),  # -> (64, 64, 64)
            nn.LeakyReLU(0.2, inplace=True),

            nn.Conv2d(feature_map_size, feature_map_size * 2, 4, 2, 1, bias=False),  # -> (128, 32, 32)
            nn.BatchNorm2d(feature_map_size * 2),
            nn.LeakyReLU(0.2, inplace=True),

            nn.Conv2d(feature_map_size * 2, feature_map_size * 4, 4, 2, 1, bias=False),  # -> (256, 16, 16)
            nn.BatchNorm2d(feature_map_size * 4),
            nn.LeakyReLU(0.2, inplace=True),

            nn.Conv2d(feature_map_size * 4, feature_map_size * 8, 4, 2, 1, bias=False),  # -> (512, 8, 8)
            nn.BatchNorm2d(feature_map_size * 8),
            nn.LeakyReLU(0.2, inplace=True),

            nn.Conv2d(feature_map_size * 8, feature_map_size * 16, 4, 2, 1, bias=False),  # -> (1024, 4, 4)
            nn.BatchNorm2d(feature_map_size * 16),
            nn.LeakyReLU(0.2, inplace=True),

            nn.Conv2d(feature_map_size * 16, 1, 4, 1, 0, bias=False),  # -> (1, 1, 1)
            nn.Sigmoid()
        )
...
class GAN(nn.Module):
    def __init__(self, latent_dim=100, img_channels=3, feature_map_size=[64,128], device="cpu"):
        super(GAN, self).__init__()
        self.latent_dim = latent_dim
        self.device = device

        # Generator
        self.generator = Generator128Deep(
        ...
        # Discriminator
        self.discriminator = Discriminator128Deep(
            img_channels=img_channels,
...
latent_dim = 100
feature_map_size_1 = 64
feature_map_size_2 = 128
num_epochs = 100
lr_d = 1e-4
lr_g = 1e-5
beta1 = 0.5
...
# === GAN model ===
gan = GAN(latent_dim=latent_dim, feature_map_size=[feature_map_size_1,feature_map_size_2], device=device)
